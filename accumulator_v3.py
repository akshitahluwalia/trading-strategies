...
class AutoAccumulator(object):
    ...
    def build_auxillary_signal_data(self):
        ...
        self.data['Heikin Ashi - T-1 Change - Negative'] = self.data['Heikin Ashi - T-1 Change'] <= 0.0
        self.data['Heikin Ashi - Buy Signal'] = self.data["Heikin Ashi - T Change - Positive"] & self.data['Heikin Ashi - T-1 Change - Negative']
        return self.data
    
    def build_indicator_data(self,supertrend_period= 10,supertrend_multiplier= 2):
        sti = ta.supertrend(self.data['High'], self.data['Low'], self.data['Close'], length= supertrend_period, multiplier=supertrend_multiplier)
    ...
    def fetch_order(self, instrument):
        order_book = self.get_orderbook_from_broker()
        filtered_orders = []
        for order in order_book:
            if(order.instrument_token == instrument and order.status == "complete" ):
                filtered_orders.append(order)
        return filtered_orders
    ...
    def run(self):
        ...
        # last_candle["Supertrend Direction"] > 0.0 and
        if  last_candle["Heikin Ashi - Buy Signal"] == True:
            self.output(f"****** Buy Signal Encountered | Placing Entry Market Orders ******")
            entry_order_id = self.place_market_order(
            ...
            self.output(f"****** Buy Signal Encountered | Order filled @ {entry_price} ******")
            stop_loss_price = self.round_nearest(last_candle["Close"] - (0.10 * last_candle["Close"]),0.05)
            stop_loss_trigger_price = stop_loss_price + 0.05
            self.output(f"****** Buy Signal Encountered | Placing Stop Loss Orders @ Price :: {stop_loss_price} and Trigger :: {stop_loss_trigger_price} ******")
            ...
            wait_time = 0
            last_quote = 0
            profit_and_loss_at_tick = 0

            while wait_time < total_sleep:
                option_quote =  self.fetch_instrument_quote(self.selected_option.instrument_key)
                root_key = list(option_quote.keys())[0]
                quote_ltp = option_quote[root_key].last_price
                last_quote = quote_ltp
                profit_and_loss_at_tick = int(self.lots) * int(self.selected_option["lot_size"]) * (last_quote - entry_price)
                self.output(f"****** Buy Signal Active | Sleeping for {wait_time}/{total_sleep} seconds :: Trade Current PNL =  {profit_and_loss_at_tick} ******")
                wait_time += 2
                time.sleep(2)

            self.output(f"****** Exiting Positions :: Current PNL :: {profit_and_loss_at_tick} ******")
            self.cancel_orders_for_instrument(self.selected_option.instrument_key)
            exit_order_id = self.place_market_order(
                    instrument= self.selected_option.instrument_key,
                    quantity= int(self.lots) * int(self.selected_option["lot_size"]),
                    transaction_type= "SELL"
            )
            ...
            order_book = self.get_orderbook_from_broker()
            for order in order_book:
                if(order.order_id == exit_order_id ):
                    exit_price = order.average_price
            slippage = int(self.lots) * int(self.selected_option["lot_size"]) * (exit_price - last_quote)
            self.output(f"****** Exiting Positions :: Current PNL :: {profit_and_loss_at_tick} and Slippage :: {slippage} ******")
        else:
            self.output(f"Buy Signal was not met :: PreviousCandleChange :: {last_candle["Heikin Ashi - T-1 Change"]} :: CurrentCandleChange :: {last_candle["Heikin Ashi - T Change"]}")
